...
Renderer.prototype.drawTriangle = function(p1, p2, p3){ //,useTexture) {
	...
	for(var x = minX; x < maxX; x++) {
		for(var y = minY; y < maxY; y++) {
			...
			var b3 = 1 - b1 - b2;
			var index = y * this.imageData.width * 4 + x * 4;
			var depthIndex = y * this.imageData.width + x;
			var depth = Math.floor(b1 * p1.z + b2 * p2.z + b3 * p3.z);
			if((b1 >= 0 && b1 <= 1) && (b2 >= 0 && b2 <= 1) && (b3 >= 0 && b3 <= 1)) {

				if(depth > this.depthMap[depthIndex]) {
					this.imageData.data[index] = b1 * p1.r + b2 * p2.r + b3 * p3.r;
					this.imageData.data[index + 1] = b1 * p1.g + b2 * p2.g + b3 * p3.g;
					this.imageData.data[index + 2] = b1 * p1.b + b2 * p2.b + b3 * p3.b;
					this.imageData.data[index + 3] = 255;
					if(b1 < .02 || b2 < .02 || b3 < .02) {
						this.imageData.data[index] = this.imageData.data[index + 1] = this.imageData.data[index + 2] = 0;
					}
					/*if(useTexture ){
